...
async function sendMessagePack(messagePack) {
    try {
        // to agent
        if (messagePack.agentUsername && messagePack.messageForAgent) {
            worldEmitter.emit(`messageFor${messagePack.agentUsername}`, messagePack.messageForAgent);
        }
        // to target
        if (messagePack.targetUsername && messagePack.messageForTarget) {
            worldEmitter.emit(`messageFor${messagePack.targetUsername}`, messagePack.messageForTarget);
        }
        // to observers
        if (messagePack.observerUsernames && messagePack.messageForObservers) {
            messagePack.observerUsernames.forEach((observerUsername) => {
                worldEmitter.emit(`messageFor${observerUsername}`, messagePack.messageForObservers);
            });
        }
    }
    catch (error) {
