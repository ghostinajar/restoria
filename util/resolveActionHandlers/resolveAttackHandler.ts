...
async function resolveAttackHandler(
  attacker: IAgent,
  defender: IAgent,
  providedRoom?: IRoom
) {
  try {
    ...
    const attackerUser = attacker as IUser;
    if (attackerUser.username) {
      messagePack.agentUsername = attackerUser.username;
    }
    const defenderUser = defender as IUser;
    ...
    if (room?.users && room.users.length > 1) {
      ...
      room.users.forEach((user) => {
        if (
          user.username !== attackerUser.username &&
    ...
    if (!hitSucceeds) {
      ...
      if (defenderUser.username) {
        ...
        messagePack.messageForTarget = {
          type: `attackMiss`,
          content: `${startWithCapitalLetter(
            attacker.name
    ...
    if (!grossDamage) {
      throw new Error(
        `couldn't calculate gross damage with attacker.rollWeaponDamage`
      );
    }
    ...
    if (defenderUser.username) {
      ...
      messagePack.messageForTarget = {
        ...
        content: `${startWithCapitalLetter(
          attacker.name
        )}'s attack hits you for ${netDamage} damage!`,
      };
    }
    if (messagePack.observerUsernames) {
      // observers include users
      messagePack.messageForObservers = {
        type: `attackMiss`,
        content: `${startWithCapitalLetter(attacker.name)}'s attack hits ${
          defender.name
        } for ${netDamage} damage!`,
      };
    }
    // console.log(
    //   `resolveAttackHandler calling sendMessagePack with messagePack:`
