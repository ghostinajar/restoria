...
function rollDice(rollParams: string) {
  try {
    ...
    if (numberOfDice < MIN_DICE || numberOfDice > MAX_DICE) {
      catchErrorHandlerForFunction(
        'rollDice',
        new Error(`Number of dice (${numberOfDice}) clamped to range ${MIN_DICE}-${MAX_DICE}`)
      );
    ...
    if (diceSides < MIN_SIDES || diceSides > MAX_SIDES) {
      catchErrorHandlerForFunction(
        'rollDice',
        new Error(`Number of sides (${diceSides}) clamped to range ${MIN_SIDES}-${MAX_SIDES}`)
      );
